 ...
 *
 * שני מקורות:
 * 1. `docs/legacy-urls.md` — אינוונטר sitemap שנאסף 2026-08-16, 83 כתובות ייחודיות.
 *    שתיים מהן (`/?jet-theme-core=`) הן query על השורש ומכוסות ממילא, ושתיים
 *    (`/` ו-`/faq/`) מצביעות על עצמן באתר החדש — נותרו 79 כללים.
 * 2. דוח הכיסוי של Search Console (2026-09-18) — כתובות שנפלו ל-404 ולא הופיעו
 *    ב-sitemap הישן: 8 נתיבים סטטיים כאן, ועוד שתי תבניות ב-`LEGACY_PATTERN_REDIRECTS`.
 *    `/elementor-1439` נותר 404 במכוון ואינו מופיע כאן.
 *
...
export const LEGACY_REDIRECTS: readonly LegacyRedirect[] = [
  ...
  /* --- דוח 404 של Search Console, 2026-09-18 --- */
  /* שרידי דף הבית של וורדפרס. Next מתאים את ה-source ללא תלות ברישיות, אבל שתי
     הצורות נשמרות במפורש כדי שהכיסוי יהיה קריא מתוך הדוח עצמו */
  { from: "/HomePage", to: "/" },
  { from: "/homepage", to: "/" },
  /* ה-stubs הרב-לשוניים של האתר הישן — מעולם לא היה בהם תוכן מתורגם */
  { from: "/en", to: "/" },
  { from: "/ru", to: "/" },
  { from: "/סוויטת-חתן-כלה", to: "/articles/bridal-suite" },
  { from: "/השכרות-לטווח-קצר", to: "/articles/short-term-furnished-rental" },
  { from: "/corporate", to: "/solutions" },
  { from: "/השכרת-חדרים-לימי-הולדת-ומסיבות-הפתעה", to: "/solutions" },
];

/**
 * כללי תבנית — נתיבים עם פרמטר, שאי אפשר לבטא כמפת נתיב-לנתיב. `next.config.ts`
 * פולט אותם **לפני** המפה הסטטית, בשתי צורות הסלאש, עם `statusCode: 301`.
 *
 * הם ASCII בלבד ולכן אינם עוברים `toLowercaseEncodedPath` — קידוד אחוזים היה בורח
 * את הנקודתיים של הפרמטר והכלל היה מפסיק להתאים.
 */
export const LEGACY_PATTERN_REDIRECTS: readonly LegacyRedirect[] = [
  /* וורדפרס מייצר feed לכל ארכיון וכל פוסט. הכלל מסיר את הסיומת ומחזיר את הנתיב
     עצמו, ומשם הבקשה נופלת למפה הסטטית (למשל `/מאמרים/feed` → `/מאמרים` → `/articles`).
     `:path+` דורש לפחות מקטע אחד לפניו, כך ש-`/feed` לבדו אינו נתפס, ואין באתר החדש
     אף מסלול שמסתיים ב-`/feed` — הכלל אינו בולע נתיב חי. */
  { from: "/:path+/feed", to: "/:path+" },
  /* עמודי הדפדוף של ארכיון המחבר. הכלל הסטטי `/author/de646f9bb8cf4378` מכסה רק את
     העמוד הראשון; `/author/<id>/page/<n>` נשאר 404 בלעדיו */
  { from: "/author/:id/page/:n", to: "/articles" },
];
