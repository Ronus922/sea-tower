/* שכבת מדידה מרכזית — נקודת הכניסה היחידה לכל תג שיווקי/אנליטי באתר.
   ...
   המזהה לא יושב בקוד אלא ב-NEXT_PUBLIC_GTM_ID: .env.local בפיתוח,
   /etc/sea-tower/sea-tower.env בייצור. אין למלא כאן מזהי דמה — בלי מזהה תקף
   השכבה כולה היא no-op ולא נטען שום סקריפט צד-שלישי.
   שימו לב: NEXT_PUBLIC_* נצרב בזמן build. החלפת מזהה מחייבת בנייה מחדש,
   לא restart לשירות.

   כללי ברזל של השכבה:
   1. אין מזהה → אין dataLayer, אין סקריפט, אין אירועים.
   2. Consent Mode v2 מאותחל לפני כל תג; הערכים עצמם ב-Analytics.tsx.
